...
NumberOfArgsSwitchT(Tf)
{
    ...
    else static if( (is( T == class ) || is( T == struct )) && is(typeof(&T.opCall)) )
        alias NumberOfArgsSwitchT!(typeof(&T.opCall)).type type;
}
...
ReturnTypeT(Tf)
{
    ...
    static if( is( typeof(*Tf) U == function ) )
        ...
        alias ReturnTypeT!(typeof(&T.opCall)).type type;
    else
...
ArgTypeSwitchT(Tf, uint n)
{
    static if( is( typeof(*Tf) == function ) )
        ...
        alias ArgTypeSwitchT!(typeof(&T.opCall)).type type;
    else
...
template MIN_ARGS_T(alias fn) {
    alias typeof(&fn) fn_t;
    static if (is(typeof(fn())))
        const uint MIN_ARGS = 0;
    else static if (is(typeof(fn(ArgType!(fn_t, 1).init))))
        const uint MIN_ARGS = 1;
    else static if (is(typeof(fn(ArgType!(fn_t, 1).init, ArgType!(fn_t, 2).init))))
        const uint MIN_ARGS = 2;
    else static if (is(typeof(fn(ArgType!(fn_t, 1).init, ArgType!(fn_t, 2).init, ArgType!(fn_t, 3).init))))
        const uint MIN_ARGS = 3;
    else static if (is(typeof(fn(ArgType!(fn_t, 1).init, ArgType!(fn_t, 2).init, ArgType!(fn_t, 3).init, ArgType!(fn_t, 4).init))))
        const uint MIN_ARGS = 4;
    else static if (is(typeof(fn(ArgType!(fn_t, 1).init, ArgType!(fn_t, 2).init, ArgType!(fn_t, 3).init, ArgType!(fn_t, 4).init, ArgType!(fn_t, 5).init))))
        const uint MIN_ARGS = 5;
    else static if (is(typeof(fn(ArgType!(fn_t, 1).init, ArgType!(fn_t, 2).init, ArgType!(fn_t, 3).init, ArgType!(fn_t, 4).init, ArgType!(fn_t, 5).init, ArgType!(fn_t, 6).init))))
        const uint MIN_ARGS = 6;
    else static if (is(typeof(fn(ArgType!(fn_t, 1).init, ArgType!(fn_t, 2).init, ArgType!(fn_t, 3).init, ArgType!(fn_t, 4).init, ArgType!(fn_t, 5).init, ArgType!(fn_t, 6).init, ArgType!(fn_t, 7).init))))
        const uint MIN_ARGS = 7;
    else static if (is(typeof(fn(ArgType!(fn_t, 1).init, ArgType!(fn_t, 2).init, ArgType!(fn_t, 3).init, ArgType!(fn_t, 4).init, ArgType!(fn_t, 5).init, ArgType!(fn_t, 6).init, ArgType!(fn_t, 7).init, ArgType!(fn_t, 8).init))))
        const uint MIN_ARGS = 8;
    else static if (is(typeof(fn(ArgType!(fn_t, 1).init, ArgType!(fn_t, 2).init, ArgType!(fn_t, 3).init, ArgType!(fn_t, 4).init, ArgType!(fn_t, 5).init, ArgType!(fn_t, 6).init, ArgType!(fn_t, 7).init, ArgType!(fn_t, 8).init, ArgType!(fn_t, 9).init))))
        const uint MIN_ARGS = 9;
    else static if (is(typeof(fn(ArgType!(fn_t, 1).init, ArgType!(fn_t, 2).init, ArgType!(fn_t, 3).init, ArgType!(fn_t, 4).init, ArgType!(fn_t, 5).init, ArgType!(fn_t, 6).init, ArgType!(fn_t, 7).init, ArgType!(fn_t, 8).init, ArgType!(fn_t, 9).init, ArgType!(fn_t, 10).init))))
        const uint MIN_ARGS = 10;
}
...
template MIN_ARGS(alias fn) {
    alias MIN_ARGS_T!(fn).MIN_ARGS MIN_ARGS;
}
